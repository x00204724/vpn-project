...
class WindowsPerfMeasure:
    def measure_baseline(self, trials=5):
        ...
        for i in range(trials):
            ...
            try:
                cmd = 'powershell -c "Invoke-WebRequest -Uri http://localhost:' + str(PORT) + '/' + ALICE_FILE + ' -OutFile test' + str(i) + '.txt -UseBasicParsing"'
                ...
                if result.returncode == 0:
                    ...
                    print(f'Trial {i+1}: FAILED - {result.stderr[:100]}')
            except Exception as e:
                print(f'Trial {i+1}: TIMEOUT - {e}')
        ...
        if times:
            ...
            result = {
                ...
                'trials': len(times),
                'avg_time_s': float(avg),
                'throughput_kbs': float(throughput),
